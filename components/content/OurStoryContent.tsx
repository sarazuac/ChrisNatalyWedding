        ...
        <div className="mx-auto max-w-3xl">
          {milestones.map((m, i) => (
            <Reveal key={m.title} delay={i * 80}>
              <div className="grid grid-cols-[auto_1fr] gap-x-8 md:gap-x-12">
                <div className="flex flex-col items-center">
                  <span className="font-display italic text-xl text-sage">
                    {m.year}
                  </span>
                  <span className="mt-3 h-3 w-3 rounded-full border-2 border-sage" />
                  {i < milestones.length - 1 && (
                    <span className="w-px flex-1 bg-line my-2" />
                  )}
                </div>
                <div className={i < milestones.length - 1 ? "pb-14" : ""}>
